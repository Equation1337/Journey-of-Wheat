...
events.onPlayerInteractBlock(function(event as crafttweaker.event.PlayerInteractBlockEvent){
    ...
    if(!isNull(event.block) && !isNull(event.item)){
        ...
        if(event.block.definition.id=="minecraft:enchanting_table"
            &&event.item.definition.id=="contenttweaker:fenge_huizhang_unstable"){
                ...
                for j in -1 as int to 2{
                    ...
                    if(check<1){
                        ...
                        for i in 0 to player.inventorySize{
                            if(!isNull(player.getInventoryStack(i))&&(player.getInventoryStack(i).definition.id=="contenttweaker:fenge_huizhang_unstable")){
                                player.replaceItemInInventory(i,<contenttweaker:fenge_huizhang>);
                                flag=true;
                            }
                        }
                        if(flag){
