        ...
        for jjj in 0 to 2{
            ...
            for i in 0 to 9{
                for j in 0 to 9{
                    ...
                    if(p=="X")continue;
                    if(!M.checkBlock(world.getBlock(M.shiftBlockPos(pos,i- 4,0,j- 4)),mapper[p]))flag=true;
                    if(flag)break;
                }
                if(flag)break;
            }
...
function checkRitual(world as IWorld, pos as IBlockPos)as string[]{
    ...
    for i in 0 to 4{
        ...
        if(!M.checkBlock(world.getBlock(pos2),"minecraft:chest")){
            ...
            if(!stacksMatch(M.getItemsInChest(world.getBlock(pos2)),CHESTS[direction[i]])){
...
events.onPlayerInteractBlock(function(event as crafttweaker.event.PlayerInteractBlockEvent){
    ...
    if(!isNull(event.block)&&event.block.definition.id=="minecraft:beacon"&&!isNull(event.item)&&event.item.definition.id=="contenttweaker:fenge_huizhang"){
            ...
            if(entity.nbt.ForgeData has tagName && getKill(player)>-1 && world.getDimension()==1){
                ...
                if(getKill(player)>99){
                    ...
                    for i in 0 to player.inventorySize{
                        if(!isNull(player.getInventoryStack(i))&&(player.getInventoryStack(i).definition.id=="contenttweaker:fenge_huizhang")){
                            player.replaceItemInInventory(i,<contenttweaker:weini_huizhang>);
                            break;
                        }
                    }
                    setKill(player,-1);
